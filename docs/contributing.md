# Contributing to `habmoti`
All contributions, bug reports, bug fixes, documentation improvements, enhancements, and ideas are welcome.
We recommend going through the list of [`issues`](https://github.com/LaboratoireELAN/HABMOT-I/issues) to find issues that interest you, preferable those tagged with `good first issue`.
...
Go to the [habmoti project page](https://github.com/LaboratoireELAN/HABMOT-I/) and hit the `Fork` button.
You will want to clone your fork to your machine:

```bash
git clone https://github.com/your-user-name/habmoti.git
```
...
- `cd` to the `habmoti` source directory
- Install `habmoti` dependencies with:

```bash
conda create -n habmoti
conda activate habmoti
conda install pip -cconda-forge
pip install . && pip uninstall -y habmoti
```
...
The `habmoti` test suite runs automatically on GitHub every time a commit is submitted.
...
`habmoti` tries to follow as much as possible the PEP recommendations (https://www.python.org/dev/peps/). 
...
`habmoti` is linted with the 120-character max per line's option. 
